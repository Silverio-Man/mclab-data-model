...
namespace Modeling {
	...
	ConditionalExpression::ConditionalExpression(std::shared_ptr<Type> type, std::shared_ptr<Expression> elseCond,
		...
		else if (elseCond->getType().lock()->getName() != then->getType().lock()->getName() &&
				 elseCond->getType().lock()->getName() != type->getName() &&
	...
	void ConditionalExpression::setElse(std::shared_ptr<Expression> &el) {
		if (el->getType().lock()->getName() != thenCondExpr->getType().lock()->getName() &&
	...
	void ConditionalExpression::setThen(std::shared_ptr<Expression> &th) {
		if (th->getType().lock()->getName() != elseCondExpr->getType().lock()->getName() &&
	...
	bool operator==(const ConditionalExpression &e1, const ConditionalExpression &e2) {
		return e1.condExpr == e2.condExpr && e1.elseCondExpr == e2.elseCondExpr && e1.thenCondExpr == e2.thenCondExpr;
	}
